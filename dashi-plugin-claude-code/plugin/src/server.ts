...
const INSTRUCTIONS_TEMPLATE = [
  'Calling the reply tool is the ONLY way the user receives your answer. Any text you write outside a reply tool call is never delivered — it stays in this transcript and the user never sees it. Every turn in this channel MUST end with a call to reply (or react/edit_message when that is the appropriate response).',
  '',
...
bot.catch(err => {
  log.error('grammy handler error (polling continues)', {
