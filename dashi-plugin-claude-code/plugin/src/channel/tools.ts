        ...
        if (args.format === 'html') {
          ...
          for (let i = 0; i < chunks.length; i++) {
            ...
            try {
              ...
              if (args.format === 'markdownv2' && isTelegramHtmlParseError(err)) {
                // Telegram rejected our MarkdownV2 escaping. Retry the SAME
                // chunk as plain text so the user still sees the answer body
                // instead of losing the whole reply — mirrors the html
                // branch above. isTelegramHtmlParseError's name predates
                // this second use site but its detection (Telegram's
                // "can't parse entities" family) isn't HTML-specific.
                log.warn('telegram MarkdownV2 parse failed, retrying as plain text', {
                  chunk_index: i,
                  error: err instanceof Error ? err.message : String(err),
                })
                const plainOpts: SendMessageOpts = {}
                if (i === 0 && replyToId !== undefined) plainOpts.reply_to_message_id = replyToId
                const sent = await telegramApi.sendMessage(args.chat_id, chunk, plainOpts)
                sentIds.push(sent.message_id)
              } else {
                throw err
              }
