...
//
// Telegram caps sendMessage at 4096 chars. We chunk at 3900 by default to
// leave headroom for grammY's reply_parameters / parse_mode overhead (also
// the limit scripts/require-reply-hook.py's fallback sender uses, so a
// dropped-reply recovery message chunks the same way a normal one would).
//
